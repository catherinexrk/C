...
//解决方式2，将声明和实现写到一起，文件后缀名改为.hpp	个人理解为cpp
#include "person.hpp"
void test01()
{
...
template<class T1, class T2>//模板不可以丢
void printPerson2(Person<T1, T2> & p)
{
...
注意！比较的时候是从头开始比较，如果前者某个位置大于被比者字母，则返回1，否则返回-1，相等返回0
...
* `erase(const_iterator start, const_iterator end);`//删除迭代器从start到end之间的元素 删除区间范围数据
* `clear();`                                                        //删除容器中所有元素
...
* 尾插  --- push_back
* 尾删  --- pop_back
* 插入  --- insert    (位置迭代器)`insert(v.begin(),num,elem)`
* 删除  --- erase  （位置迭代器）`erase(v.end())`
* 清空  ---  clear  
